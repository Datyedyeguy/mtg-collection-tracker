...
| `FlavorName`      | text        | YES      | Alternate flavor name for showcase/crossover/Universe Beyond printings (e.g., "Lightning, Lone Commando"). Null for standard printings. Used to match search queries against themed versions.                          |
...
| `Colors`          | jsonb       | YES      | JSON array of color codes: `["W","U","B","R","G"]`. Stored as JSONB for efficient queries.                                                                                                                             |
| `ImageUris`       | jsonb       | YES      | JSON object with Scryfall CDN image URLs: `{"small":"url","normal":"url","large":"url"}`. Stored as JSONB.                                                                                                             |
| `Legalities`      | jsonb       | YES      | JSON object with format legality: `{"standard":"legal","modern":"banned"}`. Values: "legal", "not_legal", "restricted", "banned". Stored as JSONB.                                                                     |
| `Faces`           | jsonb       | YES      | JSON array of card faces for multi-faced cards (transform, modal DFC, reversible). Structure: `[{"name":"...","type_line":"...","oracle_text":"..."}]`. Null for single-faced cards. See ADR-020 for design rationale. |
| `Finishes`        | jsonb       | YES      | JSON array of available finishes: `["nonfoil","foil","etched"]`. Used for validation (users can only add copies with available finishes).                                                                              |
...
- `IX_Cards_Faces` - **GIN index** on `Faces` JSONB column for efficient multi-faced card queries
- `IX_Cards_Finishes` - **GIN index** on `Finishes` JSONB column for finish availability queries
...
- Create materialized view for searching card face names (see ADR-020)
...
| `Quantity`       | integer     | NO       | How many nonfoil copies owned (0-999).                                                                    |
| `FoilQuantity`   | integer     | NO       | How many traditional foil copies owned (0-999). Default: 0.                                               |
| `EtchedQuantity` | integer     | NO       | How many etched foil copies owned (0-999). Etched foils introduced in Commander Legends 2020. Default: 0. |
...
| `AddCardFacesColumn`               | 2026-02-16 | Add Faces JSONB column for multi-faced cards with GIN index and CHECK constraint             |
